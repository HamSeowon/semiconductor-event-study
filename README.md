# Semiconductor Event Study: SK Hynix, Samsung Electronics and Micron

## Research Question
Which types of corporate/market events (corporate actions, earnings 
...
across a domestic peer (Samsung Electronics) and an international peer 
...
- **Event log:** 64 manually researched events across 4 categories 
  ...
  volatility), each with a cited news source
...
- **Statistical test:** one-sample t-test (H0: mean cumulative excess 
  return over a ±3 trading day window = 0)
...
1. **Corporate actions and earnings announcements produced statistically 
   significant excess returns for Samsung Electronics** (corporate action: 
   +5.85%, p=0.0003; earnings: +3.79%, p=0.016), but not for SK Hynix, 
   despite SK Hynix showing a *higher* raw average (+8.37% for corporate 
   actions) — the difference is explained by SK Hynix's much larger 
   standard deviation, illustrating why raw averages alone can be misleading.
...
## Limitations
- **Multiple comparisons**: 11 t-tests were run without correction 
  (e.g., Bonferroni), which increases the chance of false positives. 
...
- **Foreign/institutional flow data** (KRX investor-type trading data) 
  could not be reliably collected via the pykrx API within the project 
  timeframe; this is a natural next step.
## Tech Stack
